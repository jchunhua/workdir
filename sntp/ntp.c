...
#define NTP_SERVER_IP       "192.168.1.10" /*国家授时中心 IP*/
...
/* 设置接收NTP包的数据结构 */
         ...
         ret_time->reference_identifier = ntohl(*(int*)&(data[12]));
          ...
          ret_time->reference_timestamp.intg = ntohl(*(int*)&(data[16]));

          ret_time->reference_timestamp.frag = ntohl(*(int*)&(data[20]));

          ret_time->originage_timestamp.intg = ntohl(*(int*)&(data[24]));

          ret_time->originage_timestamp.frag = ntohl(*(int*)&(data[28]));

          ret_time->receive_timestamp.intg = ntohl(*(int*)&(data[32]));

          ret_time->receive_timestamp.frag = ntohl(*(int*)&(data[36]));

          ret_time->transmit_timestamp.intg = ntohl(*(int*)&(data[40]));

          ret_time->transmit_timestamp.frag = ntohl(*(int*)&(data[44]));
...

{
     ...
     tv.tv_sec = pnew_time_packet->transmit_timestamp.intg - JAN_1970;

     tv.tv_usec = USEC(pnew_time_packet->transmit_timestamp.frag);
